...
function getTwitterTimeline(params) {
  return new Promise((resolve, reject) => {
    client.get('statuses/user_timeline', {screen_name: params, count: 200},  function(error, tweets, response) {
      if (error) {
        reject(error);
        return;
      }
...
function writeToRedis(tweet, screen_name){
  return new Promise((resolve, reject) => {
    r_client.hmset(screen_name, 'tweet_id', tweet['id'], 'tweet_text', tweet['text'], 'avatar', tweet['user']['profile_image_url'], 'screen_name', tweet['user']['screen_name'], function (error, result) {
     if (error) {
       reject(error);
       return;
     }
